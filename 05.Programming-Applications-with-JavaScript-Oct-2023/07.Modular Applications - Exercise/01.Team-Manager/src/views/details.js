...
import { loaderTemplate } from "./utils/loader.js";
...
const memberTemplate = (request, isOwner, isSelf) => html` <li>
  ${request.user.username} ${isSelf ? "(You)" : ""}
  ${isOwner && !isSelf
    ? html` <a
        @click=${request.decline}
...
const pendingMemberTemplate = (request) => html` <li>
    ...
    function createControls() {
      if (userId !== null) {
        ...
        if (isOwner) {
          ...
          return html`<a href=${`/edit/${team._id}`} class="action"
            >Edit team</a
          >`;
        } else if (request && request.status === "member") {
          // Current user is a member
          return html`<a
            @click=${(e) => leave(e, request._id)}
            href="javascript:void(0)"
            class="action invert"
            >Leave team</a
          >`;
          ...
          return html`Membership pending.
            <a @click=${(e) => leave(e, request._id)} href="javascript:void(0)"
              >Cancel request</a
            >`;
        } else {
