function solve(arr) {
  ...
  for (let line of arr) {
    ...
    if (score > 3) {
      ...
      if (!listOfStudents.hasOwnProperty(grade)) {
        listOfStudents[grade] = [];
      }

      listOfStudents[grade].push(student);
    }
  }
  let sortedGrades = Object.keys(listOfStudents).sort((a, b) => a - b);

  for (let grade of sortedGrades) {
    let students = listOfStudents[grade];
